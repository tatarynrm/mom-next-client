...
export default function AddTransportationModal({
  ...
  const onSubmit = async (data: TransportationFormData) => {
    try {
      await api.post("/transportations", data);
      reset();
      setOpen(false);
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        <Button variant="outline">Додати перевезення</Button>
      </DialogTrigger>

      <DialogContent className="sm:max-w-lg w-full max-h-[90vh] overflow-y-auto">
        {/* max-h-[90vh] + overflow-y-auto робить модалку скролюваною на мобільних */}
        <DialogHeader>
          <DialogTitle>Додати нове перевезення</DialogTitle>
          <DialogDescription>
